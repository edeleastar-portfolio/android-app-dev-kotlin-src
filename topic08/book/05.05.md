...
If the placemark is completely new, then we need to set a default location correctly. Introduce this filed to store the default location:

~~~
  val defaultLocation = Location(52.245696, -7.139102, 15f)
~~~

And them refactor the initialization to use this when we are creating a new placemark:
...
And when we are setting a new location, we also set a default location before starting the map activity:
...
Finally, make sure we are recovering the location correctly when the MapActivity finishes:
...
This is the complete activity at this stage:
